...
oil <- read.csv("data/Brent Crude Oil.csv")
cpi <- read.csv("data/Core CPI.csv")
yield <- read.csv("data/3-month yield.csv")
```
...
```{r filter_data}
final_data <- data %>%
  select(Symbol, Name, Bankrupt, FactSet.Econ.Sector, FactSet.Ind, Entity.NAICS.Sector.Full, Year, Month, 281:302)
# dim(final_data)
...
cleaned_data <- subset_final_data %>%
  select(1:3, 6:8, "ROA", "ROE", "Working_Capital_to_Total_Assets",
         ...
         "EBITDA_Interest_Coverage_Ratio", "Cash_Flow_to_Total_Debt",
         27:30
         )
#dim(cleaned_data)
```
